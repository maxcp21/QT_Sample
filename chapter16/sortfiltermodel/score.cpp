...
void Score::populateTable()
{
    ...
    model->setHeaderData(1, Qt::Horizontal, tr("國文"));
    model->setHeaderData(2, Qt::Horizontal, tr("數學"));
    model->setHeaderData(3, Qt::Horizontal, tr("英語"));
...
void Score::setFilter()
{
        ...
        tableView->update();	// 如果不update()，表格的列標題將顯示混亂
}
