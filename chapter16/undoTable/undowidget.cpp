        ...
        : QTableWidget(parent)
{
        ...
        //itemActivated信號沒有發出，故採用currentItemChanged
...
{
    QTableWidgetItem *item = currentItem();
    // QTableWidgetItem首次返回的背景色是黑色，導致Undo時背景是黑色
...
{
        if(ignore)	// 避免Undo/Redo操作再次再次產生Push Stack命令
        {
                ignore = false;
                return;
        }
