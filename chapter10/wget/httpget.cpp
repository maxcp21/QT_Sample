...
void HttpGet::httpRequestFinished(int requestId, bool error)
{
        ...
        if (error) {
                ...
                qDebug()<<tr("文件%1 下載完畢。").arg(fileName);
        }
        delete file;
        file = 0;
}
...
void HttpGet::readResponseHeader(const QHttpResponseHeader &responseHeader)
{
        ...
        if (responseHeader.statusCode() != 200) {//請求成功。
                qDebug()<<tr("下載失敗：%1。")
...
void HttpGet::updateDataReadProgress(int bytesRead, int totalBytes)
{
        if (httpRequestAborted)
                return;
        qDebug()<<tr("已下載：%1%").arg(100*bytesRead/totalBytes);
}

void HttpGet::httpDone(bool error)
{
        if(error){
                qDebug()<<tr("錯誤:")<<qPrintable(http->errorString())<<endl;
        }
        emit done();
}
