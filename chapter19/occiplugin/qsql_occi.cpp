    ...
    : QSqlDriver(parent)
{
        ...
        {
//		setLastError(QSqlError(tr("創建環境錯誤:"), e.getMessage().c_str(), QSqlError::ConnectionError, e.getErrorCode());
...
QOCCIDriver::~QOCCIDriver()
{
    if (isOpen())
        ...
        {
//		setLastError(QSqlError(tr("釋放環境誤:"), e.getMessage().c_str(), QSqlError::ConnectionError, e.getErrorCode());
    }
}
...
//		conn = env->createConnection((UString)user.toLocal8bit().constData(), (UString)password.toLocal8bit().constData());
        ...
        {
                //setLastError(QSqlError(tr("連接錯誤"), e.getMessage().c_str(), QSqlError::ConnectionError, e.getErrorCode());
        setOpenError(true);
